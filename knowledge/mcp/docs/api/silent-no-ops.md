...
| menu position with a neighbour kind outside `menu-page` / `menu-custom-item` | `200` | the neighbour is not resolved, so the item is not placed between the two: its rank is reset to the middle of the range, and the order that results can still look like the one you asked for | name the neighbour `menu-page` or `menu-custom-item` — `page` and `custom` belong to `newParentType` in that same body, not here — and verify by the moved item's lexorank, not by the order |
