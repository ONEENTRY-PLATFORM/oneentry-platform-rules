...
| attribute set create or update with an attribute that has no `identifier` | `201` / `200` | the attribute has no marker to address it by, so it never appears in `attributeValues` on a content read, and nothing built on those values — filters, search — can see it | give every attribute in the schema an `identifier`, then read a record of that set back and confirm the attribute is present in `attributeValues` |
